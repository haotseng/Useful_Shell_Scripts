...
#
# (1) HDD 路徑&名稱
DISK1=/dev/disk/by-id/ata-VBOX_HARDDISK_VB25787b0a-0086c3d1
DISK2=/dev/disk/by-id/ata-VBOX_HARDDISK_VB7c4a7587-e89e4fdf
DISK3=/dev/disk/by-id/ata-VBOX_HARDDISK_VBe9f0e0e0-0bec5a25

# (2) ZFS UUID (可以不用修改, 用亂數產生)
ZFS_UUID=$(dd if=/dev/urandom bs=1 count=100 2>/dev/null | tr -dc 'a-z0-9' | cut -c-6)

# (3) Hostname for new system
NEW_SYSTEM_HOSTNAME=ubuntu-srv1
...
dd if=/dev/zero of=${DISK3} bs=1M count=1000
...
zfs create -o canmount=off -o mountpoint=/ \
    rpool/USERDATA
zfs create -o com.ubuntu.zsys:bootfs-datasets=rpool/ROOT/ubuntu_${ZFS_UUID} \
...
mkdosfs -F 32 -s 1 -n EFI ${DISK3}-part1
...
chroot /mnt /usr/bin/env DISK1=${DISK1} DISK2=${DISK2} DISK3=${DISK3} ZFS_UUID=${ZFS_UUID} bash -c /root/chroot_second_stage.sh
